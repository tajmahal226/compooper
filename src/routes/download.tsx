      ...
      <main id="main-content" className="site-wrap py-10 sm:py-16">
        <article className="mx-auto max-w-[520px] rounded-xl border border-card-border bg-card p-6 text-center shadow-(--shadow-sm) sm:p-10">
          <img
            ...
            alt=""
            className="mx-auto mb-4 size-[72px] object-contain"
          />
          <h1 className="mb-3 text-[clamp(1.8rem,4vw,2.2rem)] font-extrabold">
            Put Compooper on your iPhone
          </h1>
          <p className="text-muted">
            Add it to your Home Screen from Safari. Full-screen compass, no App Store, no ads — just
          ...
          <ol className="mt-6 space-y-3 text-left">
            ...
            <Step n={2} title="Tap Share">
              ...
              <span className="mt-2 inline-flex items-center gap-1.5 rounded-lg bg-brand-soft px-2 py-1 text-xs font-bold text-brand">
          ...
          <Link
            to="/map"
            className="mt-6 inline-flex h-12 w-full items-center justify-center gap-2 rounded-[14px] bg-brand px-5 font-bold text-on-brand no-underline hover:bg-brand-dark"
          >
...
function Step({ n, title, children }: { n: number; title: string; children: ReactNode }) {
  return (
    <li className="flex gap-3 rounded-2xl border border-card-border bg-bg-top/40 p-3.5">
      <span className="grid size-11 shrink-0 place-items-center rounded-full bg-brand text-sm font-extrabold text-on-brand">
        {n}
      </span>
      <span>
